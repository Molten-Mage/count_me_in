...
- [ ] Finish in-app purchase — client-side plumbing is done: `PurchaseService` (`lib/services/purchase_service.dart`) wraps the `in_app_purchase` package (query product, buy, restore, listen for purchase updates and grant `premiumStatus`), wired into the upsell dialog's real "Upgrade" button (`premium_upsell_dialog.dart`, debug builds still use the instant fake-grant toggle) and a new "Restore purchases" button in Settings. Analytics: `logPurchaseCompleted`/`logPurchaseFailed`. **Still needed once enrolled:** create the actual In-App Purchase product in App Store Connect and update `premiumProductId` in `purchase_service.dart` to match it exactly; add the "In-App Purchase" capability in Xcode; a Sandbox tester account to actually test a purchase. Purchase state is local-only (SharedPreferences, same as today) — it does *not* sync to Firestore, so a second device needs "Restore purchases" once; real cross-device sync would need server-side receipt verification (App Store Server API + a Cloud Function), deliberately left for later
